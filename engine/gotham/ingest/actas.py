...
def list_district_actas(ubigeo: int, ambito: int = 1) -> list[dict[str, Any]]:
    """Todas las actas (metadata, incl. `id`) de un distrito; sigue TODA la paginación.

    El endpoint ignora `tamanio` y pagina de a 100, así que un distrito grande puede tener
    decenas de páginas. CADA página se reintenta ante un None (throttle del CDN): si solo
    reintentáramos la primera, una página intermedia bloqueada truncaría el resto del
    distrito — la causa de que el barrido 'completo' se quedara a la mitad."""
    out: list[dict[str, Any]] = []
    page = 1
    total_pages: int | None = None
    while total_pages is None or page <= total_pages:
        d = None
        for attempt in range(6):                       # reintentar ESTA página
        ...
        if d is None:                                  # página agotó reintentos: cortar
            break
        out.extend(d["content"])
        total_pages = int(d.get("totalPaginas", 1))
        page += 1
    return out
